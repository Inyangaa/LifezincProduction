...
export function detectDistressLevel(
  ...
  if (hasHighRisk) {
    return {
      level: 'severe',
      triggers,
      recommendation: 'You may need extra support right now. There are people who care and want to help—24/7, free, and confidential.',
  ...
  if (severeCount >= 2 || (severeCount >= 1 && isNegativeMood && hasChronicPattern)) {
    return {
      level: 'high',
      triggers,
      recommendation: 'What you\'re going through is really hard. Talking to someone—a counselor, therapist, or trusted person—could make a difference.',
      shouldShowSupport: true,
    };
  }
  ...
  if (moderateCount >= 3 || (moderateCount >= 2 && recentEntryCount >= 5)) {
    return {
      level: 'moderate',
      triggers,
      recommendation: 'You\'ve been carrying a lot lately. Reaching out to someone you trust or a counselor could help lighten the load.',
      shouldShowSupport: true,
    };
  }
  ...
  if (moderateCount >= 1 || (isNegativeMood && text.length > 300)) {
    return {
      level: 'low',
      triggers,
      recommendation: 'You\'re doing a great job expressing yourself. Keep going, and remember support is here whenever you need it.',
