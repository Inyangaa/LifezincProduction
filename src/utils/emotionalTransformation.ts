import { generateIntelligentSuggestions } from './intelligentSuggestions';
...
function getIntelligentAction(moods: string[], category?: string | null): { action: string; isFallback: boolean } {
  const result = generateIntelligentSuggestions(moods, category);

  const actionSuggestions = result.suggestions.filter(s => s.type === 'action');
  if (actionSuggestions.length > 0) {
    const selected = actionSuggestions[Math.floor(Math.random() * actionSuggestions.length)];
    return {
      action: `${selected.title}: ${selected.description}`,
      isFallback: result.isFallback
    };
  }

  if (result.suggestions.length > 0) {
    const selected = result.suggestions[Math.floor(Math.random() * result.suggestions.length)];
    return {
      action: `${selected.title}: ${selected.description}`,
      isFallback: result.isFallback
    };
  ...
  return {
    action: 'Take one small step to care for yourself',
    isFallback: true
  };
}
...
export function generateEmotionalTransformation(
  ...
  const lowerMood = (Array.isArray(mood) ? mood[0] : mood).toLowerCase();
  ...
  const { action: randomAction, isFallback } = getIntelligentAction(moods, category);
